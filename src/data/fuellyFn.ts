export const fuellyFn = [
    {
      ...
      items: [
        ...
        "Skeleton UI 및 로딩바를 적용하여 데이터 페칭 중에도 매끄러운 사용자 경험 제공"
      ]
    },
    {
      ...
      items: [
        ...
        "XSS 공격 방지를 위해 Access Token은 브라우저 메모리에, Refresh Token은 HttpOnly Cookie에 저장하는 이중 보안 구조 설계",
        "Next.js Middleware를 통한 라우트 보호 및 인증 상태 기반의 선제적 리다이렉션 처리"
      ]
    },
    {
      title: "재발급 요청 중복 방지 (Race Condition)",
      items: [
    ...
    },
    {
      title: "AI 맞춤형 식단 지능",
      items: [
        "Grok AI(Groq)를 활용해 사용자의 설문 데이터와 실시간 영양 상태를 분석한 개인 맞춤형 음식 추천 시스템",
    ...
    },
    {
      title: "데이터 기반 대시보드",
      items: [
        "MongoDB와 Recharts를 연동하여 최근 일주일간의 칼로리·단백질 섭취 추이를 시각화",
